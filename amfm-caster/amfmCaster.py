    ...
    try:
        ...
        while st["connected"] and st["transmitting"]:
            ...
            try:
                ...
                log(f"[{station_key.upper()}] Send/encode error: {e}")
                break
        ...
        if encoder is not None and ws is not None:
            try:
                flushed = encoder.flush()
                if flushed:
                    ws.send_binary(flushed)
            except Exception:
                pass
...
def open_network_config_modal(station_key):
    ...
    ctk.CTkLabel(frame, text="Station Name:").grid(row=0, column=0, sticky="w", padx=10, pady=8)
    ...
    ctk.CTkLabel(frame, text="Server URL:").grid(row=1, column=0, sticky="w", padx=10, pady=8)
    ...
    modal.after(20, lambda: center_modal(modal, 560, 220))
